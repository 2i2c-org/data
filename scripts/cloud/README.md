...
- `maus-by-hub.csv`: monthly active users per hub, sampled daily. Uses the JupyterHub `jupyterhub_active_users` metric. This will double-counts users who are active on more than one hub on the same cluster.
...
Published as the `cloud-latest` GitHub release.
